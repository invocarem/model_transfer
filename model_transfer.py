...
# Try importing RDMA-capable libraries
try:
    import torch
...
class ModelTransfer:
    ...
    def transfer_rsync(self, dest_host: str, dest_user: str = None):
        ...
        dest = f"{dest_user}@{dest_host}:{self.dest_dir}" if dest_user else f"{dest_host}:{self.dest_dir}"
    ...
    def transfer_rdma(self, rank: int, world_size: int, master_addr: str, master_port: int):
        ...
        if not self.use_rdma:
            print("RDMA not available, falling back to TCP")
            return False
